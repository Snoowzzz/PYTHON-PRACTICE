...
```python
def add_score(scoreboard, name, score):
    # Adds {"name": name, "score": score} to scoreboard list
    # If name already exists, UPDATE their score only if new score is higher

def top_n(scoreboard, n):
    # Returns list of top n entries sorted by score descending
    # If n > len(scoreboard), return all entries

def has_qualified(scoreboard, threshold):
    # Returns list of names whose score >= threshold
```
...
add_score(board, "Riya", 95)
add_score(board, "Arjun", 72)
add_score(board, "Soham", 91)   # Should UPDATE Soham to 91

print(top_n(board, 2))
# Expected: [{'name': 'Riya', 'score': 95}, {'name': 'Soham', 'score': 91}]

print(has_qualified(board, 90))
# Expected: ['Riya', 'Soham']
```

> 💡 Adversarial check: `top_n(board, 100)` when board has 3 entries. `has_qualified(board, 100)` → no one qualifies.
> 💡 This is a **mini real-world system** — this exact pattern appears in leaderboards, ranking APIs, and competitive coding backends.

---

## Session 1 Log (fill after solving)
...
E4	✅	Yes	✅	Used enumerate + for-else — strongest easy of session
M1	✅	Yes	✅	Arrived at seen-tracker naturally without hints
M2	✅	Yes	✅	Self-caught hardcoded bug
M3	✅	Yes	✅	Caught mentor's question error
